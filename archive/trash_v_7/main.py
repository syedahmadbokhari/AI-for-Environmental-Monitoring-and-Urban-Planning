...
def run_pipeline(video_path: str, model_path: str, threshold: float) -> None:
    ...
    while True:
        ...
        raw_bboxes = find_bounding_boxes(fg_mask, min_area=max(1, int(bg.min_area * _scale * _scale)))
        merged_bboxes, groups = merge_nearby_bboxes(raw_bboxes, merge_gap=tracker.merge_gap * _scale)
        tracks = tracker.update(merged_bboxes, raw_bboxes=raw_bboxes, groups=groups)
        # Stage 3
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--video",     type=str,   default="videos/video.mp4", help="Path to input video.")
