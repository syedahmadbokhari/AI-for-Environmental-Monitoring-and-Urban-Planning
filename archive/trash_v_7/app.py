...
def _save_config() -> None:
    """Persist global settings and per-camera settings to disk."""
    data = {
        "settings": dict(_settings),
        "camera_settings": dict(_camera_settings),
    }
    with _config_lock:
        tmp = CONFIG_FILE + ".tmp"
...
def _load_config() -> None:
    """Load persisted settings from disk (if config file exists)."""
    ...
    if "camera_settings" in data and isinstance(data["camera_settings"], dict):
        for cam_id, overrides in data["camera_settings"].items():
...
_SETTINGS_SCHEMA: Dict[str, tuple] = {
    ...
    "process_every_n":       (int,   1,     30),
    "process_width":         (int,   320,   3840),
    "merge_gap":             (int,   1,     200),
    "fragment_min_area":     (int,   1,     10000),
    "max_event_area":        (int,   1,     100000),
}
...
def add_camera():
    ...
    cam = CameraPipeline(
        ...
        process_width=s["process_width"],
        merge_gap=s["merge_gap"],
        fragment_min_area=s["fragment_min_area"],
        max_event_area=s["max_event_area"],
        roi=tuple(cam_overrides["roi"]) if "roi" in cam_overrides else None,
    )
    cam.start()
...
def update_camera(cam_id):
    ...
    # Update per-camera settings (all settings are hot-applicable)
    if "settings" in data and isinstance(data["settings"], dict):
        new_settings = data["settings"]
        if not new_settings:
            # Empty dict means reset to global defaults
            _camera_settings.pop(cam_id, None)
            with _settings_lock:
                for key in _SETTINGS_SCHEMA:
                    if hasattr(cam, key):
                        setattr(cam, key, _settings[key])
        else:
            overrides = _camera_settings.get(cam_id, {})
            for key in _SETTINGS_SCHEMA:
                if key in new_settings:
                    if new_settings[key] is None:
                        # Explicit null means remove this override
    ...
    # ── ROI update (independent of settings dict, sent as top-level key) ──
    if "roi" in data:
        roi_val = data["roi"]
        overrides = _camera_settings.setdefault(cam_id, {})
        if roi_val is None:
            overrides.pop("roi", None)
            cam.roi = None
        else:
            # Validate: list/tuple of 4 floats each in [0, 1] with x1<x2, y1<y2
            try:
                r = [float(v) for v in roi_val]
                if (len(r) == 4
                        and all(0.0 <= v <= 1.0 for v in r)
                        and r[0] < r[2]
                        and r[1] < r[3]):
                    overrides["roi"] = r
                    cam.roi = tuple(r)
                else:
                    return jsonify({"error": "roi must be [x1,y1,x2,y2] with each value in [0,1] and x1<x2, y1<y2"}), 400
            except (TypeError, ValueError):
                return jsonify({"error": "roi must be a list of 4 numbers"}), 400
        if overrides:
            _camera_settings[cam_id] = overrides
        else:
            _camera_settings.pop(cam_id, None)
...
@app.route("/api/cameras/<cam_id>/debug_view", methods=["POST"])
def toggle_debug_view(cam_id):
    with _cameras_lock:
        cam = _cameras.get(cam_id)
    if cam is None:
        return jsonify({"error": "not found"}), 404
    data = request.get_json(silent=True) or {}
    cam.debug_view = bool(data.get("enabled", not cam.debug_view))
    return jsonify({"camera_id": cam_id, "debug_view": cam.debug_view})
...
def _load_model_global(model_path: str, threshold: float) -> None:
    ...
    # Load persisted config (settings + per-camera overrides) from disk
    _load_config()

    # Only apply CLI threshold if no config file was loaded (first run)
    with _settings_lock:
        if not os.path.exists(CONFIG_FILE):
