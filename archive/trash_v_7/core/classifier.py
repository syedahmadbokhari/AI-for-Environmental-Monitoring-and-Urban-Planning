...
_INFER_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
_INFER_STD  = np.array([0.229, 0.224, 0.225], dtype=np.float32)


def _preprocess_cv2(image_bgr: np.ndarray) -> torch.Tensor:
    """Resize + normalise using OpenCV/numpy — avoids PIL round-trip."""
    img = cv2.resize(image_bgr, (224, 224), interpolation=cv2.INTER_AREA)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = img.astype(np.float32) / 255.0
    img = (img - _INFER_MEAN) / _INFER_STD
    # HWC → CHW
...
def predict(image_bgr: np.ndarray, model, class_names, device, threshold: float):
    tensor = _preprocess_cv2(image_bgr).to(device)
