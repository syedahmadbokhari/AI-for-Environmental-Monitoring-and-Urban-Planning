        ...
        try:
            while not self._stop_event.is_set():
                ...
                if _scale is None:
                    ...
                    if orig_w > self.process_width:
                        _scale = self.process_width / orig_w
                    else:
                        _scale = 1.0
                ...
                for _, track in new_events:
                    self.total_drops += 1
                    # Scale bbox back to original resolution for classification
                    if _scale < 1.0:
                        ox, oy, ow, oh = track.bbox
                        inv = 1.0 / _scale
                        full_track_bbox = (int(ox * inv), int(oy * inv),
                                           int(ow * inv), int(oh * inv))
                        orig_bbox = track.bbox
                        track.bbox = full_track_bbox
